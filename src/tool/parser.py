...
def parse_novel_list(html: str) -> List[Dict]:
    ...
    for item in items:
        try:
            ...
            # 检查必要元素是否存在
            if not title_tag or not detail_div:
                continue

            # 确保 detail_div 中有足够的 p 标签
            ...
            novel = {
                ...
                '作者': p_tags[0].get_text().split(':')[-1].split('/')[0].strip(),
                '出版社': p_tags[0].get_text().split('/')[-1].split(':')[-1].strip(),
                '最后更新': p_tags[1].get_text().split('/')[0].split(':')[-1].strip(),
                '字数': p_tags[1].get_text().split('/')[1].strip(),
                '状态': p_tags[1].get_text().split('/')[-1].strip(),
                '标签': p_tags[2].get_text(strip=True).replace('Tags:', ''),
                '简介': p_tags[3].get_text(strip=True)
            }

            # 处理特殊状态标记
            if 'hottext' in p_tags[1].get_text():
                novel['状态'] = '已完结'
